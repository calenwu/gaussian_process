...
class Model(object):
    ...
    def __init__(self):
        ...
        kernel = 1 * RBF(length_scale=5, length_scale_bounds=(1e-10, 1e2))

        self.gaussian_process = GaussianProcessRegressor(kernel=kernel, alpha=1.5, n_restarts_optimizer=15)
    ...
    def make_predictions(self, test_x_2D: np.ndarray, test_x_AREA: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        predictions = gp_mean
        for x in range(len(gp_mean)):
            if test_x_AREA[x]:
                predictions[x] += math.sqrt(gp_std[x]) * 3.5
    ...
    def fitting_model(self, train_y: np.ndarray, train_x_2D: np.ndarray):
        ...
        x_shuffled, y_shuffled = shuffle(train_x_2D, train_y, random_state=42)
        self.gaussian_process.fit(x_shuffled[:10000], y_shuffled[:10000])
